...
login(user: string, password: string){
  ...
  loginInv(user: string, password: string){
    ...
    this.rol=parseInt(this.get('123456$#@$^@1ERF',localStorage.getItem('rol')!));
     ...
     this.updateTaskUsr(ModiFi).subscribe(
      data => {
          ...
          Swal.fire('Gracias por ver mi PortFolio')
          clearInterval(this.cl);
...
deleteTaskUsr(task:Usr):  Observable<Usr> {
  const body={title: 'Angular POST Request Example'};
  const headers = { 'Authorization': localStorage.getItem('auth_token')!, 'Content-Type': 'application/json' };
  const url = `${this.apiUrl}/user/borrar/${task.id}`;
  return this.http.delete<Usr>(url,{headers});
...
updateTaskUsrd(task:Usr): Observable<Usr>{
  const option ={
    headers: new HttpHeaders(
      {
  ...
  const body={title: 'Angular POST Request Example'};
  const url = `${this.apiUrl}/user/editard/${task.id}`;
  return this.http.put<Usr>(url+'?user='+task.user
  +'&password='+task.password
  +'&token='+task.token
  +'&expired='+task.expired
